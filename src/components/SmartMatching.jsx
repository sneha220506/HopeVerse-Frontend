...
function calculateScore(vol, need) {
  ...
  if (vol.preferredCategories?.includes(need.category)) {
    score += 35;
    reasons.push(`Prefers ${need.category}`);
  }
  const relevantSkills = CATEGORY_SKILL_MAP[need.category] || [];
  console.log(relevantSkills)
  const matched = (vol.skills || []).filter((s) =>
    relevantSkills.some(
      (rs) => s.toLowerCase().includes(rs) || rs.includes(s.toLowerCase()),
    ),
  );
  if (matched.length > 0) {
    score += Math.min(matched.length * 15, 45);
    reasons.push(`${matched.length} relevant skill(s)`);
  }
  if (vol.region === need.region || need.region === "All Zones") {
    score += 20;
  ...
  if (vol.tasksCompleted > 20) {
    score += 10;
    reasons.push("Experienced");
  } else if (vol.tasksCompleted > 5) {
    score += 4;
  }
  score +=
    { "full-time": 8, flexible: 6, "part-time": 4, weekends: 2 }[
      vol.availability
    ] || 0;
...
export default function SmartMatching({ permissions }) {
  ...
  const runMatch = () => {
    ...
    setMatches([]);
    setTimeout(() => {
      const targetNeeds = selectedNeed
        ? openNeeds.filter((n) => n._id === selectedNeed)
        : openNeeds.filter(
            (n) => (n.volunteersAssigned || 0) < (n.volunteersNeeded || 1),
          );
      const results = [];
      for (const need of targetNeeds) {
        for (const vol of available) {
          ...
          if (score >= 30)
            results.push({ volunteer: vol, need, score, reasons });
        }
      }
  ...
  return (
    <section className="py-8 bg-gradient-to-br from-slate-50 via-white to-primary min-h-screen relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-10 right-20 w-16 h-16 border-2 border-secondary/20 rounded-full animate-float animation-delay-1000" />
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8 animate-reveal">
          <div className="flex items-center gap-4 mb-4">
            <h2 className="text-4xl font-heading font-black text-slate-dark tracking-tighter">
              Smart Match <span className="text-primary">Engine</span>
            </h2>
          </div>
        </div>
        ...
        <div className="bg-primary/20 backdrop-blur-3xl rounded-[3rem] border border-white p-10 mb-12 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.05)] animate-reveal stagger-1">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-end">
            <div className="lg:col-span-5">
              <label className="text-slate-dark text-[10px] font-black uppercase tracking-[0.25em] mb-4 block ml-2">
                Analyze Target Need
              </label>
              <div className="relative">
                <select
                  ...
                  className="w-full bg-white border-2 border-slate-50 rounded-[1.5rem] px-6 py-5 text-[15px] font-bold text-slate-dark focus:ring-4 focus:ring-primary/5 focus:border-primary appearance-none outline-none shadow-sm transition-all"
                >
              ...
              <div className="flex items-center gap-5 bg-white border-2 border-slate-50 rounded-[1.5rem] px-6 py-4 shadow-sm">
                <div className="flex -space-x-3">
                  {available.slice(0, 4).map((v, idx) => (
                    <div
                      key={v._id}
                      className="w-10 h-10 rounded-full bg-white border-2 border-white flex items-center justify-center text-lg shadow-md ring-1 ring-slate-100 z-10"
                      style={{ zIndex: 10 - idx }}
                    >
                      {v.avatar}
                    </div>
                ...
                <div>
                  <span className="text-slate-dark font-black text-xl leading-none block">
                    {available.length}
                  </span>
                  <span className="text-slate-dark/30 text-[9px] font-black uppercase tracking-widest mt-1 block">
                    Deployable
                  </span>
                </div>
              </div>
            ...
            <div className="lg:col-span-3">
              <button
                onClick={runMatch}
                disabled={isMatching}
                className={`w-full py-5 rounded-[1.5rem] flex items-center justify-center gap-4 transition-all duration-500 font-black uppercase tracking-[0.2em] text-[11px] ${isMatching ? "bg-slate-100 text-slate-400" : "bg-primary text-white shadow-xl shadow-primary/25 hover:shadow-2xl hover:-translate-y-1"}`}
              >
                {isMatching ? (
                  <>
                  ...
                  <>
                    <span className="text-xl"></span>Execute Matrix Scan
                  </>
                )}
              </button>
            </div>
        ...
        <div className="relative min-h-[400px]">
            ...
            <div className="animate-reveal">
              <div className="flex items-center justify-between mb-10 px-4">
                <div className="flex items-center gap-3">
                  <h3 className="text-2xl font-heading font-black text-slate-dark tracking-tight">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {matches.map((match, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={`reveal-match stagger-${(idx % 3) + 1} group bg-white rounded-[2.5rem] p-8 border-2 transition-all duration-500 relative ${isAssigned ? "border-primary shadow-2xl shadow-primary/10 scale-95" : "border-transparent shadow-soft hover:shadow-2xl hover:border-primary/20 hover:-translate-y-2"}`}
                    >
                      <div className="absolute top-6 right-6 w-16 h-16 bg-white rounded-2xl shadow-lg border border-slate-50 flex flex-col items-center justify-center group-hover:bg-primary group-hover:border-primary transition-all duration-500">
                        <span className="text-[9px] font-black text-slate-300 uppercase leading-none mb-1 group-hover:text-white/60">
                          Match
                        </span>
                        <span
                          className={`text-2xl font-black leading-none group-hover:text-white ${match.score >= 80 ? "text-primary" : "text-slate-dark"}`}
                        >
                          {match.score}%
                        </span>
                      </div>
                      <div className="space-y-6 mb-8">
                        <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-[1.5rem] border border-slate-100 group-hover:bg-white transition-colors">
                          ...
                          <div>
                            <p className="text-slate-dark font-black text-sm tracking-tight">
                              {match.volunteer.name}
                            </p>
                            <p className="text-slate-dark/30 text-[9px] font-black uppercase tracking-[0.15em] mt-1">
                              {match.volunteer.skills?.[0]}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-4 p-4 bg-slate-900 rounded-[1.5rem] shadow-xl">
                          <span className="text-3xl">
                            {getCategoryIcon(match.need.category)}
                          </span>
                          <div className="flex-1 overflow-hidden">
                            ...
                            <div
                              className={`inline-block px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest mt-1 bg-white/10 ${getUrgencyColor(match.need.urgency)}`}
                            >
                              {match.need.urgency}
                            </div>
                          </div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-8">
                        {match.reasons.map((r, i) => (
                          <span
                            key={i}
                            className="px-3 py-1.5 bg-slate-50 rounded-lg text-[9px] font-black text-slate-400 uppercase tracking-tighter border border-slate-100"
                          >
                            {r}
                          </span>
                        ))}
                      ...
                      <button
                        onClick={() => handleConfirmDeployment(match, key)}
                        disabled={isAssigned}
                        className={`w-full py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.25em] transition-all duration-300 ${isAssigned ? "bg-success text-white shadow-lg" : "bg-primary/5 text-primary hover:bg-primary hover:text-white"}`}
                      >
                        {isAssigned
                          ? "✓ Deployment Confirmed"
                          : permissions?.canConfirmMatch
                            ? "Confirm Match"
                            : "Deploy Task"}
                      </button>
                    </div>
          ...
          {matches.length === 0 && !isMatching && (
            <div className="bg-white/40 backdrop-blur-md rounded-[4rem] border-2 border-dashed border-slate-200 p-24 text-center animate-reveal">
              <div className="w-32 h-32 bg-white rounded-[2.5rem] flex items-center justify-center text-6xl mx-auto mb-10 shadow-xl shadow-slate-200/50">
                🤖
              </div>
              <h3 className="text-3xl font-heading font-black text-slate-dark mb-4 tracking-tighter">
                Engine on Standby
              </h3>
              ...
              <div className="flex flex-wrap justify-center gap-4">
                {[
                  ...
                  <div
                    key={f}
                    className="px-6 py-2.5 bg-white border border-slate-100 text-slate-dark/30 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-sm"
                  >
                    {f}
                  </div>
                ))}
