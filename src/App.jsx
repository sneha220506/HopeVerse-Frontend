...
export default function App() {
  ...
  const isResetPath = path.startsWith("/reset/");
  const resetToken = isResetPath ? path.split("/").pop() : null;
  ...
  const renderSection = () => {
    // Priority 1: Reset Password (URL Based)
    if (isResetPath) {
      return (
        <ResetPassword
          token={resetToken}
  ...
  // BYPASS LOGIC: Agar Reset Path hai, toh in returns ko skip kar do
  if (!isResetPath) {
    if (authView === "landing")
      return (
        <LandingPage
  ...
  // Main Authenticated Layout OR Reset Password View
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-200">
      {/* Reset page par Navbar aur Status bar chhupa rahe hain */}
      {!isResetPath && (
        <>
          <Navbar
          ...
          >
            ...
            Operator: {user?.name}{" "}
            <span className="opacity-50">[{perms?.label || "Guest"}]</span>
          </div>
        </>
      )}

      <main className={`${!isResetPath ? "pt-16" : ""} bg-[#0f172a]`}>
